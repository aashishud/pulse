...
function SignupContent() {
  ...
  useEffect(() => {
    ...
    const discordUid = searchParams.get("discord_uid");
    const discordName = searchParams.get("discord_name");
    const discordHandle = searchParams.get("handle");
    
    if (discordToken && discordHandle) {
      setHandle(discordHandle);
      setLoading(true);
      signInWithCustomToken(auth, discordToken)
        ...
        .catch((err) => {
          ...
          setError(err.message || "Discord signup failed. Please try again.");
          setLoading(false);
        });
    }
  }, []);
  ...
  const handleDiscordSignup = () => {
    setError("");
    const username = handle.toLowerCase().trim();
    if (!username) { setError("Please enter a username first."); return; }
    if (!termsAccepted) { setError("Please accept the Terms of Service to continue."); return; }
    ...
    setLoading(true);
    // Check handle availability before redirecting
           ...
           <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-5">
                ...
                <button 
                  ...
                  type="button"
                  className="w-full bg-[#5865F2]/10 text-[#5865F2] font-bold py-3.5 rounded-2xl hover:bg-[#5865F2]/20 transition flex items-center justify-center gap-3 border border-[#5865F2]/20 active:scale-[0.98]"
                >
