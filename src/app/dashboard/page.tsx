...
function DashboardContent() {
  ...
  // Use NEXT_PUBLIC_ROOT_DOMAIN if set, otherwise fall back to window.location.host
  const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN || (typeof window !== 'undefined' ? window.location.host : 'www.pulsegg.in');
  const protocol = typeof window !== 'undefined' && window.location.protocol.startsWith('http:') ? 'http' : 'https';

  const profileUrl = `${protocol}://${rootDomain}/${userData?.username}`;
