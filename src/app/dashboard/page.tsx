   ...
   return (
      <div ref={selectRef} className={`relative ${className || ""}`}>
         <div 
            ...
            className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white text-sm outline-none hover:bg-black/60 cursor-pointer flex justify-between items-center transition-all focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20"
         >
            <span className="truncate pr-2">{selectedOption ? selectedOption.label : "Select an option..."}</span>
            <ChevronDown className={`w-4 h-4 transition-transform shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
         </div>
         {isOpen && (
            <div className="absolute z-[99] top-full left-0 right-0 mt-2 bg-[#18181b] border border-white/10 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto custom-scrollbar">
               {options.map((opt, i) => (
                  opt.isGroup ? (
                     <div key={i} className="px-4 py-2 mt-2 first:mt-0 text-[10px] font-bold text-zinc-500 uppercase tracking-widest bg-black/20">
                        {opt.label}
                     </div>
                  ) : (
                     <div 
                        key={i}
                        onClick={() => { if (opt.value !== undefined) onChange(opt.value); setIsOpen(false); }}
                        className={`px-4 py-3 text-sm cursor-pointer hover:bg-indigo-500/20 hover:text-indigo-300 transition-colors flex items-center justify-between ${value === opt.value ? 'bg-indigo-500/10 text-indigo-400 font-bold' : 'text-zinc-300'}`}
                     >
                     ...
                     {myCommunities.length > 0 && (
                        <section className={cardStyle}>
                           ...
                           <CustomSelect 
                              ...
                              onChange={(val) => setPrimaryCommunity(val)}
                              options={[
                                 { label: "None (Hidden)", value: "" },
                                 ...myCommunities.map(c => ({ label: `${c.name} (/c/${c.handle})`, value: c.handle }))
                              ]}
                           />
                        </section>
                     )}
                     ...
                     <section className={cardStyle}>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-black/40 p-4 rounded-2xl border border-white/5 shadow-inner">
                           ...
                           <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                              ...
                              {user?.steamId && (
                                 <button onClick={() => handleDisconnect('steam')} className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-sm font-bold bg-red-500/10 text-red-400 hover:bg-red-500/20 hover:text-red-300 transition border border-red-500/20">
                                    Disconnect
                                 </button>
                              )}
                           </div>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="p-5 bg-red-500/5 border border-red-500/20 rounded-2xl mb-6 shadow-inner">
                              ...
                              <div>
                                 ...
                                 <CustomSelect 
                                    value={gaming.valorant?.region || "na"} 
                                    onChange={(val) => setGaming({ ...gaming, valorant: { ...gaming.valorant, region: val } })}
                                    options={[
               ...
               {activeTab === 'layout' && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                     <section className={cardStyle}>
                        ...
                        {/* === Background Music and Video Section === */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                           ...
                           <div>
                              ...
                              <CustomSelect 
                                 value={theme.avatarDecoration} 
                                 onChange={(val) => setTheme({ ...theme, avatarDecoration: val, discordDecoration: "" })}
                                 options={[
                                    { label: "Standard", isGroup: true },
                                    { label: "None", value: "none" },
                                    { label: "Golden Ring", value: "gold" },
                                    { label: "Neon Pulse", value: "neon" },
                                    { label: "Glitch CSS", value: "glitch" },
                                    { label: "Fire CSS", value: "fire" },
                                    { label: "God Tier (Animated)", isGroup: true },
                                    { label: "Realistic Fire 🔥", value: "fire_god" },
                                    { label: "Lightning Storm ⚡", value: "electric_god" },
                                    { label: "King's Crown 👑", value: "crown_god" },
                                    { label: "Cat Companion 🐱", value: "cat_god" },
                                    { label: "Cherry Blossoms 🌸", value: "cherry_god" }
                                 ]}
                              />
                           </div>

                           {/* Discord Nitro Integration Box */}
                           <div className="col-span-full mt-4 p-6 bg-[#5865F2]/10 border border-[#5865F2]/20 rounded-2xl">
                              <div className="flex items-center gap-2 mb-2">
                                 <svg className="w-5 h-5 text-[#5865F2] fill-current" viewBox="0 0 24 24"><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z" /></svg>
                                 <h4 className="font-bold text-[#5865F2] text-sm">Nitro Avatar & Frame Sync</h4>
                              </div>
                              <p className="text-xs text-zinc-400 mb-4 leading-relaxed">
                                 We can pull your active Avatar and Decoration from Discord. Equip it on Discord first, then click Sync.
                              </p>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                 <button onClick={connectDiscord} className="w-full py-3 bg-[#5865F2] hover:bg-[#4752c4] text-white text-[10px] font-bold uppercase tracking-wider rounded-xl transition">
                                    1. Fetch Latest from Discord
                                 </button>
                                 {socials.discord_avatar && (
                                    <button
                                       onClick={() => setTheme({ ...theme, avatar: socials.discord_avatar })}
                                       className="w-full py-3 bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold uppercase tracking-wider rounded-xl transition border border-white/10"
                                    >
                                       2. Apply Nitro Avatar
                                    </button>
                                 )}
                                 {socials.discord_decoration && (
                                    <button
                                       onClick={() => setTheme({ ...theme, discordDecoration: socials.discord_decoration, avatarDecoration: 'none' })}
                                       className="w-full py-3 bg-white text-black hover:bg-zinc-200 text-[10px] font-bold uppercase tracking-wider rounded-xl transition"
                                    >
                                       3. Apply Nitro Frame
                                    </button>
                                 )}
                                 {theme.discordDecoration && (
                                    <button
                                       ...
                                       className="w-full py-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 text-[10px] font-bold uppercase tracking-wider rounded-xl transition"
                                    >
                                       Remove Frame
                                    </button>
                                 )}
                           ...
                           <div className="col-span-full">
                              ...
                              <CustomSelect 
                                 value={theme.cursorTrail} 
                                 onChange={(val) => setTheme({ ...theme, cursorTrail: val })}
                                 options={[
                                    { label: "Default (None)", value: "none" },
                                    { label: "Ghost Trail", value: "ghost" },
                                    { label: "Ghost Cursors ✦", value: "ghost-cursors" },
                                    { label: "Trailing Cursors ✦", value: "trailing" },
                                    { label: "Sparkles", value: "sparkle" },
