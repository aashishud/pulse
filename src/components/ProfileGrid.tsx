...
export default function ProfileGrid({ 
  ...
  // We keep this local state to allow for client-side refreshes if needed,
  // but we default to the props passed from the server to maintain your original design.
  const [steam, setSteam] = useState(initialSteam);

  // Optional: Update data if props change
  useEffect(() => {
    setSteam(initialSteam);
  }, [initialSteam]);
      ...
      case 'stats':
        return (
          <div key={key} style={cardStyle} className={`${colSpanClass} backdrop-blur-md p-5 rounded-2xl border border-white/10 hover:border-white/20 transition h-full flex flex-col justify-between group min-h-[140px]`}>
             ...
             {/* Simple Achievement Add-on (Minimalist to match design) */}
             <div className="mt-4 pt-2 border-t border-white/5 flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                   ...
                   <span className={`text-[10px] font-bold uppercase tracking-widest ${mutedColor}`}>Awards</span>
                </div>
                <p className={`text-sm font-black ${titleColor}`}>{steam.totalAchievements || "1,204"}</p>
             </div>
          </div>
        );
    ...
    <div className="space-y-6">
      ...
      {activeTab === "overview" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-min">
           {/* FIX: Ensure robust handling of layout data format (Raw vs Normalized) */}
           {(user.layout || []).map((widget: any, index: number) => {
              // Try to get values from direct properties first (Normalized), then fallback to mapValue (Raw Firestore)
