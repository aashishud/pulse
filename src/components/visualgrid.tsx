...
import { Eye, EyeOff, Maximize2, Minimize2, RotateCcw, Save, User, AlignLeft, Music, Gamepad2, Link as LinkIcon, Cpu, AlertTriangle, Info } from 'lucide-react';
...
export default function LayoutBuilder({ 
  ...
  const handleReset = () => { 
    setConfirmDialog({
      message: "Are you sure you want to reset your layout to the default Bento Grid? This will undo all your size and visibility changes.",
      onConfirm: () => setItems(DEFAULT_LAYOUT)
    }); 
  };
      ...
      {confirmDialog && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#121214] border border-white/10 rounded-[24px] p-6 w-full max-w-sm shadow-2xl flex flex-col items-center text-center">
             <div className="w-12 h-12 rounded-full mb-4 flex items-center justify-center bg-red-500/20 text-red-500 shadow-inner border border-red-500/20">
                <AlertTriangle className="w-6 h-6" />
             </div>
             <h3 className="text-xl font-black text-white mb-2 tracking-tight">Reset Layout?</h3>
             <p className="text-zinc-400 text-sm mb-6 leading-relaxed">{confirmDialog.message}</p>
             <div className="flex gap-3 w-full">
                <button onClick={() => setConfirmDialog(null)} className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition active:scale-[0.98]">Cancel</button>
                <button onClick={() => { confirmDialog.onConfirm(); setConfirmDialog(null); }} className="flex-1 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl transition active:scale-[0.98] shadow-[0_0_15px_rgba(220,38,38,0.4)]">Reset</button>
             </div>
          </div>
        </div>
      )}
