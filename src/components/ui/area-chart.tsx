...
function TooltipContent({ title, rows, children }: TooltipContentProps) {
  ...
  return (
    ...
    >
      <div className="px-3 py-2.5 bg-black border border-white/10 rounded-xl" ref={measureRef}>
        {title && (
          <div className="mb-2 font-medium text-zinc-400 text-xs uppercase tracking-widest">
            {title}
          </div>
        )}
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className="min-w-[140px] overflow-hidden rounded-xl bg-black/80 backdrop-blur-xl shadow-2xl border border-white/10"
...
function XAxisLabel({
  ...
  return (
    ...
    >
      <motion.span
        animate={{ opacity }}
        className={cn("whitespace-nowrap font-bold text-zinc-500 text-xs")}
        initial={{ opacity: 1 }}
